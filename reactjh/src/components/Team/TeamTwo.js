...
const TeamTwo = ({ title, backfont, data }) => (
    <section style={{ background: `url(${teamBg}) center center no-repeat #24182e` }} id="team">
        <div className="container">
            ...
            <div className="row mt-50">
                {data.map((member) => (
                    <TeamTwoMember key={member.id} title={member.title} role={member.role} image={member.image} social={member.social} />
                ))}
            </div>
        </div>
    </section>
